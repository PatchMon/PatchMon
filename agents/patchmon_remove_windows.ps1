...
try {
    ...
    if ($service) {
        if ($service.Status -eq "Running") {
            ...
            $service = Get-Service -Name $serviceName -ErrorAction SilentlyContinue
            if ($service.Status -eq "Running") {
                Write-Warning "⚠️  Service is STILL RUNNING after stop command! Attempting force stop..."
        ...
        if (-not $service) {
            ...
            Write-Warning "⚠️  Service still exists after deletion attempt"
        }
    } else {
...
if ($processes) {
    ...
    if ($remaining) {
        Write-Warning "⚠️  Some processes are still running after stop command"
    } else {
