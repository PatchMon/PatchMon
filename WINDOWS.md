...
PatchMon supports Windows hosts alongside Linux. A lightweight agent runs as a Windows Service and reports package (Windows Update) and repository (update source) information to your PatchMon server. Optionally, you can open a browser-based RDP session to Windows hosts from the host detail page (see [In-browser RDP (optional)](#in-browser-rdp-optional)).
...
The agent runs as a Windows Service and connects **outbound** to the PatchMon server. No inbound ports are opened on the Windows host.

```
┌─────────────────────────────────────┐     HTTPS (outbound)      ┌─────────────────────────────────────┐
│  Windows host                        │ ───────────────────────► │  PatchMon server                     │
│  • PatchMon agent (Windows Service)  │   install script,         │  • Backend (API, install endpoint,   │
│  • Reports: packages, repos,        │   agent binary, reports    │    host/package/repo storage)       │
│    system info                       │ ◄─────────────────────── │  • Frontend (UI)                     │
└─────────────────────────────────────┘   schedule / on demand    └─────────────────────────────────────┘
```

### In-browser RDP (optional)

When Guacamole and guacd are deployed alongside PatchMon, the UI can open an RDP session to a Windows host. The browser talks to Guacamole’s tunnel; Guacamole/guacd connect to the Windows host over RDP.

```
┌──────────────┐     /guacamole/tunnel      ┌──────────────┐     /api/tokens + RDP params     ┌──────────────┐
│  Browser     │ ◄────────────────────────► │  Guacamole   │ ◄───────────────────────────────► │  PatchMon    │
│  (RDP tab)   │     HTTP tunnel            │  (web app)   │     Backend issues ticket       │  Backend     │
└──────────────┘                            └──────┬───────┘                                  └──────────────┘
                                                   │
                                                   │ guacd protocol
                                                   ▼
...
PatchMon can open a **browser-based Remote Desktop (RDP)** session to Windows hosts from the host detail page. No separate RDP client is required; the session runs in the UI via [Apache Guacamole](https://guacamole.apache.org/).

### Requirements

- **PatchMon server:** Guacamole and guacd must be deployed and wired to the backend (see below).
- **Windows host:** Remote Desktop enabled and reachable from the server running Guacamole/guacd (typically port **3389**). If the host is on a different network, ensure firewall allows RDP from the Guacamole host.

**Enable RDP on Windows (if needed):**
...
The backend and frontend must be configured to talk to Guacamole:

1. **Run Guacamole and guacd**  
   For development, `docker/docker-compose.dev.yml` includes `guacamole-db`, `guacd`, and `guacamole` services. For production, deploy [Apache Guacamole](https://guacamole.apache.org/doc/gug/installing-guacamole.html) and guacd (e.g. via Docker or packages) so the PatchMon backend can reach the Guacamole HTTP API.

2. **Backend environment variables**
   - `GUACAMOLE_URL` – URL the backend uses to reach Guacamole (e.g. `http://guacamole:8080`).
   - `GUACAMOLE_JSON_SECRET_KEY` – 32-character hex key; must match the key configured in Guacamole for JSON authentication (e.g. in `guacamole-auth-json` / `JSON_SECRET_KEY`).

3. **Frontend**  
   The UI must be able to reach the Guacamole tunnel (e.g. same origin `/guacamole` or a reverse proxy). The dev compose and Vite proxy are set up so the app uses `/guacamole` on the same host.

### Using RDP in the UI

1. Open **Hosts** → select a **Windows** host.
2. On the host detail page, open the **RDP** tab (only visible for Windows hosts).
3. Enter the Windows username and password (and domain if the host is domain-joined).
4. Click **Connect RDP**. The remote desktop appears in the browser; you can use the **fullscreen** control for a larger view and **Disconnect** when done.

Credentials are sent to the backend only to obtain a short-lived Guacamole session; they are not stored.
...
- **RDP tab shows black screen or "connection failed"**  
  Ensure Remote Desktop is enabled on the Windows host and the firewall allows port 3389. From the machine running Guacamole/guacd, confirm the Windows host is reachable (e.g. `telnet` or `nc` to port 3389). Check that `GUACAMOLE_JSON_SECRET_KEY` matches between the PatchMon backend and Guacamole.
