...
import { ChevronRight, PlayCircle, RefreshCw } from "lucide-react";
import { Link } from "react-router-dom";
import { PatchRunStatusBadge } from "../../PatchRunStatusBadge";

const DISPLAY_LIMIT = 5;

const PatchingActivePolicies = ({ data }) => {
	const active_runs = data?.active_runs || [];
	const display_runs = active_runs.slice(0, DISPLAY_LIMIT);
	const has_more = active_runs.length > DISPLAY_LIMIT;
		...
		<div className="card p-4 sm:p-6 w-full h-full flex flex-col">
			...
			{active_runs.length === 0 ? (
				...
				<div className="flex flex-col flex-1 min-h-0">
					...
					>
						{has_more
							? `View all ${active_runs.length} active runs`
							: "View all runs"}
...
export default PatchingActivePolicies;
